...
@Component({
  selector: 'app-eg-sharp-asharp-cdsharp',
  templateUrl: './eg-sharp-asharp-cdsharp.component.html',
  styleUrls: ['./eg-sharp-asharp-cdsharp.component.scss']
})
export class EgSharpAsharpCDSharpComponent {
      ...
      this.allChords.forEach((elem2:any, ind2:number) => {
        if(ind2 > 24 && ind2 < 48) {
          if(elem.name === elem2.name) {
            ...
            elem.duurit.push(this.allChords[ind2+22])

            elem.mollit.push(this.allChords[ind2-5])
            elem.mollit.push(this.allChords[ind2+1])
            elem.mollit.push(this.allChords[ind2+7])
            elem.mollit.push(this.allChords[ind2+13])
            elem.mollit.push(this.allChords[ind2+21])
          }
        }
      })
    })

    this.backend.setEgsharpasharpcdsharp(this.chords)

    this.backend.egsharpasharpcdsharpSointuKulkuDataOsbserver.subscribe((response:any) => {
      if(response) {
        setTimeout(() => {
          this.data = response
