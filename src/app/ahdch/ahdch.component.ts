...
@Component({
  selector: 'app-ahdch',
  templateUrl: './ahdch.component.html',
  styleUrls: ['./ahdch.component.scss']
})
export class AhdchComponent {
  ...
  constructor(
      ...
      this.allChords.forEach((elem2:any, ind2:number) => {
        if(ind2 > 24 && ind2 < 48) {
          if(elem.name === elem2.name) {
            ...
            elem.duurit.push(this.allChords[ind2+4])
    ...
    this.backend.setAhdchSointuKulkuData(this.chords)

    this.backend.ahdchSointuKulkuData.subscribe((response:any) => {
      if(response) {
        setTimeout(() => {
          this.data = response
