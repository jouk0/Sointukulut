...
@Component({
  selector: 'app-eahcsharp',
  templateUrl: './eahcsharp.component.html',
  styleUrls: ['./eahcsharp.component.scss']
})
export class EahcsharpComponent {
      ...
      this.allChords.forEach((elem2:any, ind2:number) => {
        if(ind2 > 24 && ind2 < 48) {
          if(elem.name === elem2.name) {
            ...
            elem.duurit.push(this.allChords[ind2+18])

            elem.mollit.push(this.allChords[ind2-5])
            elem.mollit.push(this.allChords[ind2+4])
            elem.mollit.push(this.allChords[ind2+8])
            elem.mollit.push(this.allChords[ind2+12])
          }
        }
      })
    })

    this.backend.setEahcSharpSointuKulkuData(this.chords)

    this.backend.eahcSharpSointuKulkuData.subscribe((response:any) => {
      if(response) {
        setTimeout(() => {
          this.data = response
